...
def achievements(session: Session = Depends(get_db_session), user: AuthenticatedUser = Depends(get_current_user)) -> dict:
    ...
    available_courses = {link.course.id: link.course for link in links if link.course.is_published and link.learning_path.is_published and link.learning_path.academy.is_published and allows(link.course.groups, user) and allows(link.learning_path.academy.groups, user)}
