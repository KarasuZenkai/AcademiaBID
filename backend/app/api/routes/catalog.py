...
from app.services.access import academy_allowed, course_allowed, course_allowed_in_path, course_unlocked, module_allowed, path_allowed
from app.services.sequencing import unlocked_lesson_ids

router = APIRouter(prefix="/api", tags=["catalog"])


def academy_card(session: Session, academy: Academy, user: AuthenticatedUser) -> dict:
    paths = [path for path in academy.learning_paths if path.is_published] if "learning_paths" in academy.__dict__ else []
    visible_paths = [path for path in paths if path_allowed(session, path, user)]
    courses = [link.course for path in visible_paths for link in path.course_links if link.course.is_published and course_allowed_in_path(session, link.course, path, user)]
    lessons = [lesson for course in courses for module in course.modules for lesson in module.lessons]
    return {"id": str(academy.id), "name": academy.name, "slug": academy.slug, "description": academy.description, "image_url": academy.image_url, "learning_path_count": len(visible_paths), "content_count": len(lessons), "video_count": sum(lesson.lesson_type.value == "VIDEO" for lesson in lessons)}
...
def academies(session: Session = Depends(get_db_session), user: AuthenticatedUser = Depends(get_current_user)) -> list[dict]:
    ...
    return [academy_card(session, academy, user) for academy in records if academy_allowed(session, academy, user)]
...
def academy_detail(slug: str, session: Session = Depends(get_db_session), user: AuthenticatedUser = Depends(get_current_user)) -> dict:
    ...
    if academy is None or not academy_allowed(session, academy, user):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Academy not found")
    paths = [path for path in academy.learning_paths if path.is_published and path_allowed(session, path, user)]
    return {**academy_card(session, academy, user), "learning_paths": [{"name": path.name, "slug": path.slug, "description": path.description, "position": path.position, "course_count": len([link for link in path.course_links if link.course.is_published and course_allowed_in_path(session, link.course, path, user)]), "content_count": sum(len(module.lessons) for link in path.course_links if link.course.is_published and course_allowed_in_path(session, link.course, path, user) for module in link.course.modules if module_allowed(session, module, user))} for path in sorted(paths, key=lambda item: item.position)]}
...
def learning_path_detail(slug: str, session: Session = Depends(get_db_session), user: AuthenticatedUser = Depends(get_current_user)) -> dict:
    ...
    if path is None or not path_allowed(session, path, user):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Learning path not found")
    courses = [{"title": link.course.title, "slug": link.course.slug, "description": link.course.description, "estimated_minutes": link.course.estimated_minutes, "position": link.position, "content_count": sum(len(module.lessons) for module in link.course.modules if module_allowed(session, module, user)), "video_count": sum(lesson.lesson_type.value == "VIDEO" for module in link.course.modules if module_allowed(session, module, user) for lesson in module.lessons), "unlocked": course_unlocked(session, link.course, user)} for link in sorted(path.course_links, key=lambda item: item.position) if link.course.is_published and course_allowed_in_path(session, link.course, path, user)]
    return {"name": path.name, "slug": path.slug, "description": path.description, "academy": academy_card(session, path.academy, user), "courses": courses}
...
def course_detail(slug: str, session: Session = Depends(get_db_session), user: AuthenticatedUser = Depends(get_current_user)) -> dict:
    ...
    if course is None or not course_allowed(session, course, user):
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Course not found")
    if not course_unlocked(session, course, user):
    ...
    visible_modules = [module for module in course.modules if module_allowed(session, module, user)]
    lessons = [lesson for module in visible_modules for lesson in module.lessons]
    progress_by_lesson = {}
    records = []
    if lessons:
